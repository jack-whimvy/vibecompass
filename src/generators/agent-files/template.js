...
export function renderSharedInstructionBody(context, options = {}) {
  ...
  return [
    ...
    '- Git binding is opt-in per lane (D-281): `start-session --branch <name> --repo <id> [--worktree]` creates or reuses the branch in each bound repo, and `--worktree` provisions per-repo worktrees under `<workspace>/worktrees/<lane-id>/<repo-id>` with the lane marker in the container — commands run from inside a worktree need neither `--root` nor `--session`. Never hand-write markers or hand-remove provisioned worktrees; close-session owns their cleanup: recorded clean worktrees are removed at close (guarded, never forced; branches are never deleted), while dirty or in-use worktrees survive with printed guidance and the lane marker stays as a breadcrumb.',
    '- Lane runtime isolation is package-assigned (D-282): consume the lane port and temp dir via `eval "$(vibecompass lane-env)"` (exports conventional `PORT`/`TMPDIR` aliases) instead of hardcoding ports in lane work; close-session removes the lane temp dir under guards.',
    ...
    '- Keep decisions append-only in `decisions/`; append accepted architectural decisions before implementing them. `decisions/INDEX.md` is derived (D-283): `vibecompass append-decision` refreshes it when the lane context is resolvable, and `vibecompass refresh-decision-index` covers hand-appends — do not hand-edit index rows away from canonical headings.',
